  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final truncatedText =
            !_isExpanded
                ? '${widget.text.substring(0, endPosition.offset)}${widget.text.length < 300 ? '' : '....'}'
                : widget.text;
          ...
          child: Column(
            ...
            children: [
              ...
              if (!widget.isSuffixButton &&
                  (widget.allowCollapse || !_isExpanded) &&
                  showButton)
                widget.readMoreButtonBuilder != null
                    ? widget.readMoreButtonBuilder!(
                      _isExpanded,
                      widget.readMoreText,
                      widget.readLessText,
                      _toggleExpanded,
...
class _ExpandableTextSpan extends StatelessWidget {
  ...
  TextSpan _buildExpandCollapseTextSpan(String text) {
    return TextSpan(
      text: text,
      style: buttonTextStyle ?? const TextStyle(fontWeight: FontWeight.bold),
...
class ReadMoreTextButton extends StatelessWidget {
  /// Creates a ReadMoreTextButton widget.
  ///
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Padding(
        ...
        child: TextButton(
          onPressed: onPressed,
          child: Text(isExpanded ? readLessText : readMoreText),
        ),
      ),
    );
  }
